...
class LLMService:
    ...
    token = Config.HUGGINGFACEHUB_API_TOKEN or os.getenv("HF_TOKEN") or os.getenv("HUGGINGFACE_TOKEN")
    ...
    def __init__(self, vectorstore, page1_text: str = ""):
        ...
        if LLMService._llm is None:
            print(f"🔑 Token present: {bool(Config.HUGGINGFACEHUB_API_TOKEN)}")
            print(f"🔑 Token value starts with: {str(Config.HUGGINGFACEHUB_API_TOKEN)[:8] if Config.HUGGINGFACEHUB_API_TOKEN else 'NONE'}")
